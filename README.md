...
```bash
tofino-sam$ sam build --use-container
```
...
```bash
tofino-sam$ sam logs -n StockCheckerFunction --stack-name tofino-sam --tail
```
...
```bash
tofino-sam$ pip install -r tests/requirements.txt --user
# unit test
tofino-sam$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
tofino-sam$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name tofino-sam
```
